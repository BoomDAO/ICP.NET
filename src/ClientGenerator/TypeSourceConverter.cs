...
namespace EdjCase.ICP.ClientGenerator
{
    ...
    {
        public static (List<DeclaredTypeSourceDescriptor> Types, Dictionary<string, string> Aliases) Build(CandidServiceFile serviceFile, string didFileName)
        {
            Dictionary<CandidId, CandidType> types = serviceFile.DeclaredTypes;
            bool serviceIsInDeclaredTypes = serviceFile.ServiceReferenceId != null;

            // Only write client once
            if (!serviceIsInDeclaredTypes)
            {
                // TODO
                //types.Add(CandidId.Parse(didFileName), serviceFile.Service);
            }
            var converter = new TypeSourceConverter();
                    ...
                    default:
                        {
                            ...
                            {
                                CandidRecordType r => this.ResolveRecord(typeName.Name, r, declaredFullTypeNames),
                                CandidVariantType v => this.ResolveVariant(typeName.Name, v, declaredFullTypeNames),
                                CandidServiceType s => this.ResolveService(typeName.Name, s, declaredFullTypeNames),
                                _ => throw new NotImplementedException()
                            };
        ...
        private ServiceSourceDescriptor ResolveService(string name, CandidServiceType type, Dictionary<CandidId, TypeName> declaredFullTypeNames)
        {
            ...
            List<ServiceSourceDescriptor.Method> resolvedMethods = type.Methods
                .Select(f =>
                {
                    ...
                    (ServiceSourceDescriptor.Method method, List<TypeSourceDescriptor> innerSubTypesToCreate) = this.ResolveFunc(funcName, f.Value, declaredFullTypeNames);
        ...
        private (ServiceSourceDescriptor.Method Method, List<TypeSourceDescriptor> SubTypesToCreate) ResolveFunc(string name, CandidFuncType type, Dictionary<CandidId, TypeName> declaredFullTypeNames)
        {
            var subTypesToCreate = new List<TypeSourceDescriptor>();
            List<(string Name, string FullTypeName)> resolvedParameters = type.ArgTypes
                .Select((f, i) =>
                {
                    ...
                    return (argName, typeName?.FullTypeName);
                })
                .Where(f => f.FullTypeName != null)
                .Select(f => (f.argName, f.FullTypeName!))
                .ToList();
            List<(string Name, string FullTypeName)> resolvedReturnParameters = type.ReturnTypes
                .Select((f, i) =>
                {
            ...
            var method = new ServiceSourceDescriptor.Method(name, isFireAndForget, isQuery, resolvedParameters, resolvedReturnParameters);
